...
public class Curl {
    ...
    private HttpRequestBase createMethod(String type, String url, String data, boolean upload, boolean multipart) throws UnsupportedEncodingException {
        ...
        // Si requete PUT
        if (type.equals("PUT")) {
            method = new HttpPut(url); //incluse dans library
            if (data != null) { // s'il y a des data e poster
                ((HttpPut) method).setEntity(new StringEntity(data));
                // on transforme la methode en str
            }
        } else if (type.equals("POST")) {
            method = new HttpPost(url);
            if (data != null && !upload) { // s'il y a des data et que upload est false
                ((HttpPost) method).setEntity(new StringEntity(data));
                lastRequestBodyString = data;
            } else if (data != null && upload) { // s'il y a upload true
                if (multipart) {
